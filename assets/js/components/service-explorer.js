export function initServiceExplorer(config) {
  ...
  const arrowIcon = '<span class="home-explorer__button-icon" aria-hidden="true"><svg viewBox="0 0 24 24"><path d="M7 17 17 7" /><path d="M9 7h8v8" /></svg></span>';
  nav.innerHTML = config.services.map((service, index) => `<button class="home-explorer__button" type="button" role="tab" aria-selected="${index === 0}" data-slug="${service.slug}"><span>${String(index + 1).padStart(2, '0')}</span><span>${service.title}</span>${arrowIcon}</button>`).join('');
