...
export function renderFooter(config){
  ...
  host.innerHTML=`<div class="v-container--wide"><div class="site-footer__top"><div class="site-footer__brand"><a class="site-logo" href="index.html">${logoMarkup}<span class="site-logo__word">${escapeHTML(config.brand.name)}</span></a><p>${escapeHTML(config.footer.description)}</p><a class="v-button v-button--primary" href="contact.html">${escapeHTML(config.ctas.consultation)}</a></div><div><h3>${escapeHTML(config.footer.servicesHeading||'Design options')}</h3><ul>${services}</ul></div><div><h3>${escapeHTML(config.footer.companyHeading||config.brand.name)}</h3><ul><li><a href="about.html">About</a></li><li><a href="all-services.html">All Services</a></li><li><a href="gallery.html">Project Gallery</a></li><li><a href="contact.html">Contact</a></li></ul></div><div class="site-footer__contact"><h3>${escapeHTML(config.footer.legalHeading||'Information')}</h3><ul>${legal}</ul><p><a href="${config.contact.emailHref}">${escapeHTML(config.contact.email)}</a></p><p>${escapeHTML(config.contact.addressLine1)}<br>${escapeHTML(config.contact.cityStateZip)}<br>${escapeHTML(config.contact.country)}</p></div></div><div class="site-footer__disclaimer"><p>${escapeHTML(config.footer.disclaimer)}</p><span>${escapeHTML(config.footer.copyright)}</span></div></div>`;
}
